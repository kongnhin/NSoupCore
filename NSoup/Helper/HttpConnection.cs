...
namespace NSoup.Helper
{
	...
	public abstract class ConnectionBase<T> : IConnectionBase<T> where T : IConnectionBase<T>
	{
		...
		{
			return HasHeader(name) && Header(name).Equals(value, StringComparison.OrdinalIgnoreCase);
		}
	...
	public class Response : ConnectionBase<IResponse>, IResponse
	{
		...
		public static async Task<Response> Execute(IRequest req, IResponse previousResponse)
		{
			...
			try
			{
				if (req.Method() == NSoup.Method.Post)
				{
                    ...
                    WritePost(req.Data(), await conn.GetRequestStreamAsync());
				}
			...
			catch (WebException e)
			{
				...
				if (response != null)
				{
					return await ProcessResponse(response, req, previousResponse);
				}
			}
		...
		private static Task<Response> ProcessResponse(HttpWebResponse response, IRequest req, IResponse previousResponse)
		{
